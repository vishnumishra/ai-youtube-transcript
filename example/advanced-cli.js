...
let httpProxy = null;
let httpsProxy = null;
let webshareUsername = null;
let websharePassword = null;
...
if (webshareUsername && websharePassword) {
  ...
  proxyConfig = new GenericProxyConfig(httpProxy, httpsProxy);
}

// Create YouTube transcript instance
const ytTranscript = new YoutubeTranscript(cookiePath, proxyConfig);
...
async function listAvailableTranscripts(videoId) {
...
async function fetchTranscripts(videoIds) {
  const results = [];
